...
1. **List.** `node scripts/drift.mjs ../<repo>` from jig. The `DRIFT` rows and any `NOT YOURS` row are the whole worklist. Nothing crosses that drift did not name.
2. **Target is ready.** The target repo's tree is clean and on `main` or a branch the operator names — its `main` may be held by a worktree, so cut the branch from `origin/main`. The copy commits there, so a dirty tree means committing someone's in-progress work.
3. **Copy and delete.** `cp` each `DRIFT` file from jig; `git rm` each `NOT YOURS` file. Then `drift.mjs` again: it must say `nothing differs` before going on.
4. **Run every gate the target runs**, not the one that seems relevant. Read the target's `package.json` `verify` script and run each `check:*` in it. This is the step that was skipped, twice, and it is the point of the checklist.
5. **Sort the red.** A gate red on the target's own prose or config is the target's content, which the sync surfaced — the operator or a target session fixes it, and the sync pull request can carry it or wait. A gate red on a file the sync just copied is a template defect: fix it in jig first, land that, and re-sync — never patch the copy in the target, which forks it.
6. **Commit and open the pull request.** Only once drift says `nothing differs` and every gate is green. The body names the jig commit synced from and lists what crossed.
