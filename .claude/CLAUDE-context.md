...
| "Nothing syncs" means no automation, not "do not copy" | A jig session is the hand that moves templates, in either direction, when the operator asks. One read the phrase as a prohibition and refused a sync. The full rule is under What We're Building; the condition to check first is that the target repo's tree is clean, since the copy commits there |
